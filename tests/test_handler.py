...
from kjfwd_bot.transport import ForwardAction, MessageEvent, ReplyAction

from kjfwd_bot.capabilities import CapabilityRegistry
from kjfwd_bot.config import MessageReminderConfig, ReplyDebounceConfig
from kjfwd_bot.handler import (
    KJFWDHandler,
...
class HandlerTests(unittest.TestCase):
    ...
    def test_unanswered_latest_question_emits_message_reminder(self):
        ...
        self.handler.classifier = FakeClassifier([True])
        self.handler.handle(
            MessageEvent(
                "客户群", "电脑开不了机怎么办", time.time(), "柯基服务队", False,
                FakeRaw((40, 1)),
            )
        )
        deadline = time.time() + 1
        while not self.actions and time.time() < deadline:
            time.sleep(0.01)
        self.assertEqual(1, len(self.actions))
        self.assertIsInstance(self.actions[0], ForwardAction)
        self.assertEqual("值班提醒群", self.actions[0].target_name)
        self.assertEqual(
            "【消息提醒】客户群中有新消息，请注意处理",
            self.actions[0].content,
        )

    def test_new_message_cancels_pending_message_reminder(self):
        self.handler.message_reminder = MessageReminderConfig(
            enabled=True,
            group="值班提醒群",
            source_groups=("客户群",),
        ...
        self.handler.classifier = FakeClassifier([True, False])
        now = time.time()
        self.handler.handle(
            MessageEvent(
                "客户群", "电脑开不了机怎么办", now, "柯基服务队", False,
                FakeRaw((41, 1)),
            )
        )
        self.handler.handle(
            MessageEvent(
                "客户群", "我来处理", now + 1, "柯基服务队", False,
                FakeRaw((41, 2)),
            )
        )
        time.sleep(0.2)
    ...
    def test_message_outside_reminder_source_groups_is_not_managed(self):
        self.handler.message_reminder = MessageReminderConfig(
            enabled=True,
            group="值班提醒群",
            source_groups=("二群",),
        ...
        time.sleep(0.1)
        self.assertEqual([], self.handler.classifier.calls)
        self.assertEqual([], self.actions)
