...
from .transport import ForwardAction, MessageEvent, MessageHandler, ReplyAction
...
from .config import (
    ConversationPoolConfig,
    ImageContextConfig,
    MessageReminderConfig,
    ReplyDebounceConfig,
)
...
def event_source_key(event: MessageEvent) -> Optional[str]:
    stable_key = str(getattr(event, "source_key", "") or "").strip()
    if stable_key:
        return stable_key
...
def action_client_id(kind: str, numeric_id: int, target: str) -> str:
    """Build a stable bowxt-compatible ASCII idempotency key."""

    target_hash = hashlib.sha256(str(target).encode("utf-8")).hexdigest()[:12]
    safe_kind = re.sub(r"[^A-Za-z0-9_-]", "-", str(kind))[:20] or "action"
    return f"kjfwd-{safe_kind}-{int(numeric_id)}-{target_hash}"
...
class KJFWDHandler(MessageHandler):
    ...
    def __init__(
        ...
        *,
        ...
        message_reminder: MessageReminderConfig = MessageReminderConfig(),
        ...
        self.message_reminder = message_reminder
        ...
        self._pending_message_reminders: Dict[str, Tuple[int, threading.Timer]] = {}
    ...
    def handle(self, event: MessageEvent):
        ...
        message, inserted = self.history.record_group_message(
            event.group,
            event.content,
        ...
        recent_messages = self.history.group_recent_messages(
            event.group, before_id=message.id, limit=8
        )
        reminder_sources = self.message_reminder.source_groups
        reminder_managed = not reminder_sources or event.group in reminder_sources
        if self._image_should_trigger(event, message):
            question_decision = True
            request = "请结合这张图片和当前会话继续分析。"
            logger.info(
                "图片续接触发：group=%s sender=%s message_id=%s",
                event.group,
                event.sender or "unknown",
                message.id,
            )
        if (
            inserted
            ...
            question_decision = self._classify_message(
            ...
            if question_decision:
                self._schedule_message_reminder(event.group, message.id)
        ...
        *,
        ...
        if message.message_type == "image":
            recent = [
                ...
                if timestamp - item.updated_at <= self.image_context.lookback_seconds
            ]
            same_sender = (
                [
                    item
                    for item in recent
                    if self.history.conversation_last_sender(item.id) == sender
                ]
                if sender
                else recent
            )
            if len(same_sender) == 1:
                route = ConversationRoute(
                    "use_existing",
                    conversation_id=same_sender[0].id,
                    reason="image_followup_same_sender" if sender else "image_followup_single_recent",
                )
            elif recent:
                route = ConversationRoute("ambiguous", reason="image_followup_multiple_candidates")
            else:
                route = ConversationRoute(
                    "create_new", title="图片问题", reason="image_followup_no_candidates"
                )
        elif is_low_information_followup(request):
            recent = [
                item
                for item in candidates
            ...
            if len(same_sender) == 1:
                reason = (
                    "low_information_same_sender"
                    if sender
                    else "low_information_single_recent"
                )
                route = ConversationRoute(
                    "use_existing", conversation_id=same_sender[0].id, reason=reason
                )
            elif sender and recent and not same_sender:
                route = ConversationRoute(
                    "ambiguous", reason="low_information_different_sender"
                )
            elif candidates:
            ...
            for target_group in reply_groups or self._reply_groups_for(group_name):
                self._emit_action(
                    ReplyAction(
                        ...
                        client_id=action_client_id("direct", trigger_id, target_group),
                    )
                )
                        ...
                        self._emit_action(
                            ReplyAction(
                                ...
                                client_id=action_client_id(
                                    "reply", job.trigger_id, target_group
                                ),
                            )
                        )
                    # sent 表示 bowxt 已离开 pending；微信本身仍不提供对端送达回执。
    ...
    def _should_trigger(
        ...
        *,
        ...
        if is_at_me:
            ...
            if decision is None:
                decision = self._classify_message(
                    group_name=group_name,
                    content=request,
                    sender=sender,
    ...
    def _image_should_trigger(self, event: MessageEvent, message) -> bool:
        if (
            event.message_type != "image"
            or not self.image_context.enabled
            or not self.image_context.trigger_images
        ):
            return False
        candidates = self.history.list_active_conversations(
            event.group,
            now=message.observed_at,
            ttl_seconds=self.image_context.lookback_seconds,
            limit=self.conversation_pool.max_active,
        )
        if event.sender:
            candidates = tuple(
                item
                for item in candidates
                if self.history.conversation_last_sender(item.id) == event.sender
            )
    ...
    def _cancel_message_reminder(self, group_name: str) -> None:
        with self._context_lock:
            pending = self._pending_message_reminders.pop(group_name, None)
        if pending:
            pending[1].cancel()
            logger.info(
                "消息提醒已取消：group=%s reason=newer_message", group_name
            )

    def _schedule_message_reminder(self, group_name: str, message_id: int) -> None:
        delay = self.message_reminder.delay_seconds
        timer = threading.Timer(
            delay, self._emit_message_reminder, args=(group_name, message_id)
        )
        timer.daemon = True
        with self._context_lock:
            self._pending_message_reminders[group_name] = (message_id, timer)
        timer.start()
        logger.info(
            "消息提醒计时开始：group=%s message_id=%s delay_seconds=%s",
            group_name,
            message_id,
            delay,
        )

    def _emit_message_reminder(self, group_name: str, message_id: int) -> None:
        with self._context_lock:
            pending = self._pending_message_reminders.get(group_name)
            if not pending or pending[0] != message_id:
                return
            self._pending_message_reminders.pop(group_name, None)
            emitter = self._emit_action
        if self._stop_event.is_set() or emitter is None:
            return
        emitter(
            ForwardAction(
                target_name=self.message_reminder.group,
                target_type="group",
                content=f"【消息提醒】{group_name}中有新消息，请注意处理",
                source_group=group_name,
                client_id=action_client_id("reminder", message_id, group_name),
            )
        )
        logger.info(
            "消息提醒已入队：source_group=%s reminder_group=%s message_id=%s",
            group_name,
            self.message_reminder.group,
            message_id,
        )
    ...
    def stop(self) -> None:
        ...
        with self._context_lock:
            ...
            reminder_timers = list(self._pending_message_reminders.values())
            self._pending_message_reminders.clear()
        for timer, job in pending:
            timer.cancel()
            self.history.mark_trigger_failed(job.trigger_id, "stopped")
        for _message_id, timer in reminder_timers:
            timer.cancel()
