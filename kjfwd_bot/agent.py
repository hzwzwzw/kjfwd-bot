...
class ToolCallingAgent:
    ...
    def complete(
        self, system_prompt: str, user_prompt: Any, *, force_search: bool = False
    ) -> str:
        logger.info(
...
def should_require_search(user_prompt: Any) -> bool:
    prompt_text = text_content(user_prompt)
    match = CURRENT_REQUEST_RE.search(prompt_text)
    request = match.group(1) if match else prompt_text
...
def is_detailed_request(user_prompt: Any) -> bool:
    prompt_text = text_content(user_prompt)
    match = CURRENT_REQUEST_RE.search(prompt_text)
    request = match.group(1) if match else prompt_text
...
def text_content(user_prompt: Any) -> str:
...
def _request_excerpt(user_prompt: Any, limit: int = 120) -> str:
    prompt_text = text_content(user_prompt)
    match = CURRENT_REQUEST_RE.search(prompt_text)
    request = match.group(1) if match else prompt_text
