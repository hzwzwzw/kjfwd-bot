...
class HistoryStore:
    ...
    def _create_schema(self) -> None:
        ...
        self._ensure_column("messages", "image_sha256", "TEXT")
        self._conn.execute(
    ...
    def record_assistant_message(
        ...
        with self._lock:
            ...
            cursor = self._conn.execute(
                "INSERT INTO messages(group_name, role, content, observed_at, session_id, conversation_id, sender) "
                "VALUES (?, 'assistant', ?, ?, ?, ?, 'bot')",
    ...
    def group_recent_messages(
        self, group_name: str, *, before_id: Optional[int] = None, limit: int = 8
    ) -> Tuple[StoredMessage, ...]:
        query = "SELECT * FROM messages WHERE group_name=?"
    ...
    def conversation_last_sender(self, conversation_id: str) -> Optional[str]:
        """Return the most recent non-bot sender bound to a conversation."""

        with self._lock:
            row = self._conn.execute(
                "SELECT sender FROM messages WHERE conversation_id=? AND role='group' "
                "AND sender IS NOT NULL AND sender<>'' ORDER BY id DESC LIMIT 1",
                (conversation_id,),
            ).fetchone()
